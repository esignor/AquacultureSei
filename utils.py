...
# Applica la proiezione dei profili di cromatina su uno spazio di numerosita' delle sequence classes (ottenute da clustering/PCA durante l'addestramento di Sei):
...
# Calcola la differenza tra variante ref e alt proiettata nello spazio PCA normalizzando solo sugli istoni
def sc_hnorm_varianteffect(chromatin_profile_ref, chromatin_profile_alt, clustervfeat, histone_inds):
    ...
    diffproj = altproj[:,:n_subclass] - refproj[:,:n_subclass]
    return diffproj
...
# Scrive file .tsv con:
#Le proiezioni sulle classi di sequenza (output_sequence_class_file)
#Le predizioni grezze dei profili epigenetici (output_chromatin_profile_file)
#Ordinati secondo il massimo effetto assoluto della variante sulla classe di sequenza.
...
def write_to_tsv(max_abs_diff,
    ...
    # --- MY MODIFY: truncate to top N rows ---
    #TOP_N = 20000
    #sei_df = sei_df.head(TOP_N)
    #sc_df = sc_df.head(TOP_N)
    #print(f"Writing top {TOP_N} variants to TSV...")
