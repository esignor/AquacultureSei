...
if __name__ == "__main__":
    ...
    configs = load_path("./model/sei_seq_prediction_dlabrax2021.yml", instantiate=False)
    ...
    if not seq_input.endswith('.fa') and not seq_input.endswith('.fasta'):
        ...
        print('hg_version', hg_version)
        genome = None
        if hg_version == 'hg38' or hg_version == 'hg19' or hg_version == 'dlabrax' or hg_version == "maximus":
            genome = Genome(
                os.path.join('.', 'resources', '{0}_UCSC.fa'.format(hg_version)))
            configs["analyze_sequences"].bind(reference_sequence=genome)
        else:
            raise ValueError("--genome=<hg> must be 'hg19', 'hg38' or 'dlabrax' or 'maximus")
