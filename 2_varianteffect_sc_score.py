...
if __name__ == "__main__":
    ...
    sei_dir = "./model"
    chromatin_profiles = get_targets(os.path.join(sei_dir, "target_dlabrax2021.names")) # chromatin profiles
    seqclass_names = get_targets(os.path.join(sei_dir, "seqclass_dlabrax2021.names")) # sequence class

    clustervfeat = np.load(os.path.join(sei_dir, 'projvec_targets_dlabrax2021.npy')) # aggregated profiles with PCA, knn and Louvain
    histone_inds = np.load(os.path.join(sei_dir, 'histone_inds_dlabrax2021.npy')) # indexing histone marks
